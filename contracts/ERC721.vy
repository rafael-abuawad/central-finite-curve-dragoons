...
name: public(immutable(String[29]))
...
# @dev Returns the maximum supply of the
# token collection.
# @notice See comment on lower case letters
# above at `name`.
...
@external
@payable
def __init__(name_: String[29], symbol_: String[5], max_supply_: uint256, max_balance_: uint256, base_uri_: String[80], name_eip712_: String[50], version_eip712_: String[20]):
    """
    ...
    @notice The `owner` role will be assigned to
            the `msg.sender`.
    @param name_ The maximum 29-character user-readable string
...
def safe_mint(owner: address, uri: String[432]):
    ...
    @notice Only authorised minters can access this function.
            ...
            There is a maximum supply of tokens. If this is
            reached the function will revert.
    @param owner The 20-byte owner address.
...
@internal
@view
def _meets_balance_limits(owner: address):
    """
    @dev Reverts if the `owner` has more reached the maximum
         balance limit.
    @notice Note that `owner` cannot be the zero address.
    @param owner The 20-byte owner address.
    @return uint256 The 32-byte token amount owned
    ...
    """
    assert owner != empty(address), "ERC721: the zero address is not a valid owner"
    assert self._balance_of(owner) < max_balance, "ERC721: operation exceeds collection maximum owner balance"
